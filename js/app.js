...
$(document).ready(function () {
	...
	$('.icon-menu').click(function (event) {
		$('.icon-menu,.menu__body').toggleClass('_active'); $('.icon-menu');
		$('body').toggleClass('_lock');
	});
	$('.menu__body ul li a').click(function (event) {
...
function myFn(str, numberOfWords) { // получаем столько слов сколько нужно
	let index;
	for (let i = 0; i < numberOfWords; i++) {
	  index = str.indexOf(' ', index + 1);
	}
	return '<span>' + str.substring(0, index) + '</span>' + str.substring(index);
 }
 
 function wrapFirstWords (params) {
	const titleText = document.querySelector('.mainscreen__title').innerText;
	const titleEl = document.querySelector('.mainscreen__title');
	titleEl.innerHTML = myFn(titleText, 7);
 }
